...
export const usePrepaymentInvoiceForm = ({
  fetchCompanyData,
  initialPrepaymentInvoice,
}) => {
  const [companyData, setCompanyData] = useState(initialPrepaymentInvoice);
  ...
  }, [fetchCompanyData, initialPrepaymentInvoice]);

  const onSubmit = useCallback(async (fields) => {
    delete fields.companyInitialData;
    try {
      await fetch(`${process.env.REACT_APP_URL}/prepaymentInvoices`, {
        method: "POST",
        headers: {
